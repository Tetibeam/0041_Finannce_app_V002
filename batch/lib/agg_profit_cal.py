...
def get_asset_name(accounts, asset_name_key, sub_type_filters=None):
    # 基本的な安全対策
...
# 預金の実現損益を計算
# 特徴：収支データに登録されている
def cal_realized_ordinary_deposit(df_asset_profit, df_balance_raw):
    # 普通預金
    ...
    df_reg["資産名"] = df_reg["保有金融機関"].apply(lambda x: get_asset_name(x, "普通"))
...
def cal_realized_hybrid_deposit(df_asset_profit, df_balance_raw):
    # ハイブリッド預金
    ...
    df_reg = df_merged[["date", "保有金融機関", "実現損益"]].copy()
    df_reg["資産名"] = df_reg["保有金融機関"].apply(lambda x: get_asset_name(x, "ハイブリッド"))
    df_reg.drop("保有金融機関",axis=1, inplace=True)
...
def cal_realized_fixed_deposit(df_asset_profit, df_balance_raw):
    # 利息
    ...
    df_merged["資産名"] = df_merged["保有金融機関"].apply(lambda x: get_asset_name(x, "定期"))
    ...
    df_in.loc[~mask_account, "資産名"] = df_in.loc[~mask_account,"保有金融機関"].apply(lambda x: get_asset_name(x, "定期"))
...
def cal_realized_structured_deposit(df_asset_profit, df_balance_raw):
    # 仕組預⾦
    ...
    df_reg = df_merged[["date", "保有金融機関", "実現損益"]].copy()
    df_reg["資産名"] = df_reg["保有金融機関"].apply(lambda x: get_asset_name(x, "仕組"))
    df_reg.drop("保有金融機関",axis=1, inplace=True)
...
def set_realized_deposit(df_asset_profit, df_balance_raw):
    return (
        df_asset_profit
        .pipe(safe_pipe(cal_realized_ordinary_deposit, df_balance_raw))
        .pipe(safe_pipe(cal_realized_hybrid_deposit, df_balance_raw))
        .pipe(safe_pipe(cal_realized_fixed_deposit, df_balance_raw))
        .pipe(safe_pipe(cal_realized_structured_deposit, df_balance_raw))
    )
...
def set_realized_interest(df_asset_profit,df_balance_raw):
    ...
    df_in["資産名"] = df_in.apply(
        lambda row: get_asset_name(row["保有金融機関"], row["内容"]),
        axis=1
    )
...
def set_realized_dividend(df_balance_raw):
    ...
    df_in["保有金融機関"] = pd.Series(dtype="object")
    df_in["資産名"] = df_in.apply(
        lambda row: get_asset_name(row["保有金融機関"], row["メモ"],["国内株式","セキュリティートークン"]),
        axis=1
    )
    return df_in[["date", "資産名", "実現損益"]]

def set_realized_capital(df_asset_profit,df_balance_raw):
    # 対象資産名
...
def set_realized_dividend_and_capital(df_asset_profit,df_balance_raw):
    ...
    # 売却損益
    df_reg_capital = set_realized_capital(df_asset_profit,df_balance_raw)
    # 結合
...
def prepare_in_out(df_balance_raw, account):
    # 入出金額
    if account == "クラウドバンク":
...
def set_realized_cloud_funds(df_asset_profit, start_date, end_date, df_balance_raw):
    ...
    for account in accounts:
        ...
        #print(df_current_account_balance)
        df_in_out = (
            prepare_in_out(df_balance_raw, account)
